...
describe('App strategy navigation', () => {
  ...
  it('runs the selected strategy scanner and shows strategy-specific recommendations', async () => {
    ...
    const firstMcdCell = within(results).getByText('MCD')
    expect(firstMcdCell).toHaveAttribute('data-mobile-symbol', 'true')
    expect(firstMcdCell.closest('tr')).toHaveClass('scanner-mobile-row')
    expect(firstMcdCell.closest('tr')?.querySelector('[data-label="Current"]')).toBeInTheDocument()
    expect(firstMcdCell.closest('tr')?.querySelector('[data-label="RSI14"]')).toBeInTheDocument()
    expect(firstMcdCell.closest('tr')?.querySelector('[data-label="52W range"]')).toBeInTheDocument()
