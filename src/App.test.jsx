...
describe('App strategy navigation', () => {
  ...
  it('loads the universe page as an indicator table beside the selectable chart panel', () => {
    ...
    const universeTiles = screen.getByLabelText(/Liquid options universe ticker tiles/i)
    expect(universeTiles).toBeInTheDocument()
    expect(within(universeTiles).getByText('SPY')).toBeInTheDocument()
    expect(within(universeTiles).getByText(/SPDR S&P 500 ETF/i)).toBeInTheDocument()

    const xlpButton = within(universeTiles).getByRole('button', { name: /Select XLP/i })
    const xlpTile = xlpButton.closest('.indicator-tile')
    expect(within(xlpTile).getByText(/Consumer Staples SPDR/i)).toBeInTheDocument()
    expect(within(xlpTile).getByText(/ETF \/ index/i)).toBeInTheDocument()
    fireEvent.click(xlpButton)
    expect(xlpTile).toHaveClass('active')
    const xlpInlineChart = xlpTile.querySelector('.tile-chart-panel')
    ...
    const xlvButton = within(universeTiles).getByRole('button', { name: /Select XLV/i })
    const xlvTile = xlvButton.closest('.indicator-tile')
    fireEvent.click(xlvButton)
    expect(xlpTile).not.toHaveClass('active')
    expect(xlvTile).toHaveClass('active')
    expect(xlpTile.querySelector('.tile-chart-panel')).not.toBeInTheDocument()
    const xlvInlineChart = xlvTile.querySelector('.tile-chart-panel')
  ...
  it('runs the selected strategy scanner and shows strategy-specific recommendations', async () => {
    ...
    const firstMcdRow = within(results).getAllByRole('row').find((row) => row.textContent.includes('MCD'))
    const firstMcdCell = firstMcdRow.querySelector('[data-mobile-symbol="true"]')
    expect(firstMcdCell).toHaveAttribute('data-mobile-symbol', 'true')
    expect(firstMcdRow).toHaveClass('scanner-mobile-row')
    expect(firstMcdRow.querySelector('.mobile-row-title')).toBeInTheDocument()
    expect(firstMcdRow.querySelector('.mobile-row-chips')).toBeInTheDocument()
    expect(firstMcdRow.querySelector('[data-label="Symbol"]')).not.toBeInTheDocument()
    expect(firstMcdRow.querySelector('[data-label="Current"]')).toBeInTheDocument()
    expect(firstMcdRow.querySelector('[data-label="RSI14"]')).toBeInTheDocument()
    expect(firstMcdRow.querySelector('[data-label="52W range"]')).toBeInTheDocument()
    const scannerTiles = within(results).getByLabelText(/Scanner result tiles/i)
    const mcdTile = within(scannerTiles).getByText('MCD').closest('.indicator-tile')
    expect(mcdTile).toBeInTheDocument()
    expect(within(mcdTile).getByText(/McDonald/i)).toBeInTheDocument()
    expect(within(mcdTile).getByText(/Score/i)).toBeInTheDocument()
    expect(within(mcdTile).getByText(/RSI/i)).toBeInTheDocument()
    expect(within(mcdTile).getByText(/Current/i)).toBeInTheDocument()
    expect(within(mcdTile).getByText(/52W range/i)).toBeInTheDocument()
    ...
    expect(within(results).getAllByText('MCD').length).toBeGreaterThan(0)
    ...
    expect(within(results).getAllByText(/#13/).length).toBeGreaterThan(0)
  })
})
