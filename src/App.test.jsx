...
describe('App strategy navigation', () => {
  ...
  it('loads the universe page as an indicator table beside the selectable chart panel', () => {
    ...
    const xlpButton = within(universeTable).getAllByRole('button', { name: /Select XLP/i })[0]
    const xlpRow = xlpButton.closest('tr')
    expect(within(xlpRow).getByText(/Consumer Staples SPDR/i)).toBeInTheDocument()
    expect(within(xlpRow).getByText(/ETF \/ index/i)).toBeInTheDocument()
    expect(xlpRow.querySelector('.mobile-row-title')).toBeInTheDocument()
